...
const Navbar = () => {
  ...
  return (
    <nav
      ref={navRef}
      className={`fixed top-0 w-full z-[100] transition-all duration-500 ${isScrolled ? "py-2" : "py-6"}`}
    >
      <div className="max-w-7xl mx-auto px-6">
        <div
          className={`glass-card flex items-center justify-between px-6 py-2 transition-all duration-500 border border-white/10 ${
            isScrolled
              ? "shadow-2xl bg-base-100/80 backdrop-blur-lg border-primary/20"
        ...
        >
          ...
          <div className="flex items-center gap-10">
            <Link href="/" className="text-2xl font-bold tracking-tighter">
              ...
              <span className="text-base-content font-light italic">
                .beauty
              </span>
            </Link>
          ...
          <div className="flex items-center gap-4">
            <button className="btn btn-ghost btn-circle btn-sm hidden md:flex">
              <Search size={18} />
            </button>
            ...
            {!isLoggedIn ? (
              <div className="hidden sm:flex items-center gap-3">
                ...
                <Link
                  href="/register"
                  className="btn btn-primary btn-sm rounded-full px-6 shadow-lg shadow-primary/20 border-none"
                >
                  Register
                </Link>
              </div>
              ...
              <div className="dropdown dropdown-end">
                ...
                <ul
                  tabIndex={0}
                  className="dropdown-content z-1 mt-4 p-2 shadow-2xl bg-base-100 border border-base-300 rounded-2xl w-64 backdrop-blur-xl"
                >
                  <li className="px-4 py-3 border-b border-base-200 mb-2">
                    <p className="font-bold text-sm text-primary">
                      {user?.name}
                    </p>
                    <p className="text-[10px] text-base-content/50 truncate uppercase">
                      {user?.email}
                    </p>
                  </li>
                  <li>
                    <Link
                      href="/dashboard"
                      className="flex items-center gap-3 p-3 hover:bg-base-200 rounded-xl text-sm"
                    >
                  ...
                  <li>
                    <Link
                      ...
                      className="flex items-center gap-3 p-3 hover:bg-base-200 rounded-xl text-sm"
                    >
                      <History size={16} className="text-primary" /> Order
                      History
                    </Link>
                  </li>
                  ...
                  {/* {isAdmin && ( */}
                  <>
                    <div className="divider opacity-30 my-1 italic text-[9px] uppercase tracking-[0.2em] font-bold">
                      Manager Vault
                    </div>
                    <li>
                      <Link
                        href="/add-product"
                        className="flex items-center gap-3 p-3 hover:bg-primary/5 text-primary rounded-xl text-sm font-bold"
                      >
                  ...
                  <li>
                    <button
                      ...
                      className="flex w-full items-center gap-3 p-3 text-error hover:bg-error/5 rounded-xl text-sm font-semibold"
                    >
            ...
            <Link
              href="/cart"
              className="btn btn-primary btn-sm rounded-full gap-2 shadow-lg shadow-primary/20 border-none"
            >
              <ShoppingBag size={18} />
                    ...
                    <div className="py-3">
                      ...
                      <AnimatePresence>
                        {mobileShopOpen && (
                          ...
                          >
                            <p className="text-[10px] uppercase font-bold text-primary pt-2">
                              Categories
                            </p>
              ...
              {!isLoggedIn && (
                <div className="flex flex-col gap-3 pt-4">
                  <Link
                    href="/login"
                    onClick={() => setIsOpen(false)}
                    className="btn btn-outline border-primary/20 rounded-full"
                  >
                    Login
                  </Link>
                  <Link
